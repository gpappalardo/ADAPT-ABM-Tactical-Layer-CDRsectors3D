...
int _change_flvl_MULTI(Aircraft_t *f,Aircraft_t **flight,int N_f,CONF_t conf,TOOL_f tl,SHOCK_t sh,long double t){
	int unsafe = (*f).st_indx-1;
	...
	for(hp=40;1;hp+=20){
		/*It tries first +20FL*/
		if(!_try_flvl(f,flight,N_f,conf,tl,sh,f_lvl_on+hp,unsafe,endp,t)){
...
int _check_safe_events_MULTI_FL(Aircraft_t **f, int N_f, SHOCK_t sh,TOOL_f tl, CONF_t conf, long double t, SECTOR_t **sec){
	...
	for(i=1;i<N_f;i++) if((*f)[i].ready){
		...
		if(unsafe) {
			...
			if(unsafe) 
				/*try to chenge the flight level*/
				unsafe=_change_flvl_MULTI(&(*f)[i],f,i,conf,tl,sh,t);
...
int _evaluate_workload(Aircraft_t **f,int n_f,int N_f,TOOL_f tl,CONF_t conf, long double curr_t){
	...
	for(i=0 ;i<n_f;i++) {
		...
		for(j=(*f)[i].st_indx;j<((*f)[i].n_nvp-1);j++) {
			...
			#endif
			if( t > 3600. ) break;
			if((int) (*f)[i].nvp[j][4] >=(conf).n_sect) BuG("To Many Sector on M1 File\n");
...
int ABM(Aircraft_t **f, int N_f, CONF_t conf, SHOCK_t sh, SECTOR_t *sec){
	...
	for(t=conf.start_datetime,step=0 ;t<=NT; t+=t_stp, step++ ) if(_evolution(f,N_f, conf,sh,tool,t,&sec) == 0) {
