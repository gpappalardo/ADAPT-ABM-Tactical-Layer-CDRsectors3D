...
void plot(Aircraft_t f,CONF_t c,char *fr){
	...
	for(i=0;i<f.n_nvp;i++) {
		#ifdef EUCLIDEAN
		fprintf(wstream, "%Lf\t%Lf\t%Lf\n",f.nvp[i][0],f.nvp[i][1],f.nvp[i][4]);
		#else
...
void plot_pos(Aircraft_t f,CONF_t c,char *file_w){
	...
	int i;
	#ifdef EUCLIDEAN
	for(i=0;i<(c.t_d);i++) fprintf(wstream,"%Lf\t%Lf\n",f.pos[i][0],f.pos[i][1]);
	#else
	long double p[2];
	for(i=0;i<(c.t_d);i++){
		if(f.pos[i][0]==SAFE) continue;
...
void plot_timestep(Aircraft_t f,CONF_t c,char *file_w){
	//plot(f,c,"/tmp/pp");
	FILE *wstream=fopen(file_w,"w");
	int i;
	#ifdef EUCLIDEAN
	for(i=0;i<((int) c.t_w*c.t_r);i++) fprintf(wstream,"%Lf\t%Lf\n",f.pos[i][0],f.pos[i][1]);
	#else
	long double p[2];
	for(i=0;i<((int) c.t_w*c.t_r);i++){
		if(f.pos[i][0]==SAFE) continue;
		//gall_peter(f.pos[i], p);
		fprintf(wstream,"%Lf\t%Lf\n",f.pos[i][0],f.pos[i][1]);
	}
...
void plot_sector(SECTOR_t s,char *file_w){
	int i;
	FILE *wstream = fopen(file_w,"w");
	if(wstream==NULL) BuG("Impossible to write\n");
	
	for(i=0;i<s.n_side;i++) fprintf(wstream,"%Lf\t%Lf\n",s.bound[i][0],s.bound[i][1]);
	fclose(wstream);
	return;
}
...
int plot_point(long double *p,char *file_w){
	FILE *wstream = fopen(file_w,"w");
	if(wstream == NULL) BuG("Impossible to write\n");
...
int plot_nvp_and_pos(Aircraft_t f,CONF_t conf){
	plot(f,conf,"/tmp/nvp.dat");
	plot_pos(f,conf,"/tmp/looka.dat");
	plot_timestep(f,conf,"/tmp/timestep.dat");
